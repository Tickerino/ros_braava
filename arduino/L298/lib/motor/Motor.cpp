...
void Motor::calculate_rpm(long current_encoder_ticks)
{
    ...
    double delta_ticks = current_encoder_ticks - previous_encoder_ticks;
    //calculate wheel's speed (RPM)
    current_rpm = (delta_ticks / double(counts_per_rev))/ dtm;
    previous_encoder_ticks = current_encoder_ticks;
...
void Motor::calculate_pwm()
{
  ...
  pwm = (Kp * error) + (Ki * _total_pid_error) + (Kd * (error - previous_pid_error));
  previous_pid_error = error;
